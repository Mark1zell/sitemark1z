(function () {
  ...
  function showLoading(message = 'Загрузка...') {
    if (!loadingOverlay) {
      ...
      loadingOverlay.innerHTML = `
        <div class="mkz-loading-spinner"></div>
        <div class="mkz-loading-message">${escapeHtml(message)}</div>
      `;
  ...
  function showNotification(message, type = 'info') {
    ...
    notification.innerHTML = `
      <div class="mkz-notification__content">
        <span class="mkz-notification__message">${escapeHtml(message)}</span>
        <button class="mkz-notification__close">&times;</button>
      </div>
    `;
    ...
    const closeBtn = notification.querySelector('.mkz-notification__close');
  ...
  function validateFile(file, maxSizeMB = 5, allowedTypes = ['image/jpeg', 'image/png', 'image/webp', 'image/gif']) {
    if (file.size > maxSizeMB * 1024 * 1024) {
      throw new Error(`Файл не должен превышать ${maxSizeMB}MB`);
    }
    ...
    try {
      ...
      $$('[data-review-reply-form]', reviewsList).forEach(form => {
        form.addEventListener('submit', async function (e) {
          ...
          if (!text) {
            showNotification('Напиши ответ', 'warning');
            return;
          }
  ...
  async function likeReview(reviewId) {
    ...
    if (existing) {
      ...
      if (error) {
        showNotification(error.message || 'Не удалось убрать лайк', 'error');
        return;
      }
    } else {
      const { error } = await supabaseClient
      ...
      if (error) {
        showNotification(error.message || 'Не удалось поставить лайк', 'error');
        return;
      }
    }
  ...
  async function handleReviewSend() {
    if (!state.currentSession) {
      showNotification('Чтобы оставить отзыв, сначала войди в аккаунт', 'warning');
      openScreen('account');
      return;
    }
    ...
    try {
      ...
      if (file) {
        try {
          ...
          throw new Error('Не удалось загрузить картинку к отзыву');
        }
      }
      ...
      if (error) {
        throw new Error('Не удалось оставить отзыв: ' + error.message);
      }
    ...
    try {
      ...
      $$('[data-comment-form]', newsList).forEach(form => {
        form.addEventListener('submit', async e => {
          ...
          if (!text) {
            showNotification('Напиши комментарий', 'warning');
            return;
          }
          ...
          if (error) {
            showNotification('Ошибка комментария: ' + error.message, 'error');
            return;
          }
      ...
      $$('[data-edit-news-comment]', newsList).forEach(btn => {
        btn.addEventListener('click', async () => {
          ...
          if (error) {
            showNotification(error.message || 'Не удалось обновить комментарий', 'error');
            return;
          }
      ...
      $$('[data-poll-option]', newsList).forEach(btn => {
        btn.addEventListener('click', async () => {
          ...
          if (samePollVotes.length) {
            showNotification('Ты уже голосовал в этом опросе', 'warning');
            return;
          }
          ...
          if (error) {
            showNotification(error.message || 'Не удалось проголосовать', 'error');
            return;
          }
  ...
  async function handleDeleteNewsComment(commentId) {
    ...
    if (!canDelete) {
      showNotification('Ты не можешь удалить этот комментарий', 'warning');
      return;
    }
    ...
    if (error) {
      showNotification(error.message || 'Не удалось удалить комментарий', 'error');
      return;
    }
  ...
  async function handleDeleteNewsPost(postId) {
    ...
    try {
      ...
      if (error) {
        showNotification(error.message || 'Не удалось удалить пост', 'error');
        return;
      }
  ...
  async function handleEditNewsPost(postId) {
    ...
    if (error) {
      showNotification(error.message || 'Не удалось обновить пост', 'error');
      return;
    }
  ...
  async function handleAddNewsPost() {
    ...
    if (!normalizedText && !hasContest && !hasPoll && !hasLink && !imageFile && !extraFile) {
      showNotification('Заполни хотя бы один блок: текст, конкурс, опрос, ссылку, фото или файл', 'warning');
      return;
    }
    ...
    try {
      ...
      if (postError || !insertedPost) {
        showNotification(postError?.message || 'Не удалось создать пост', 'error');
        return;
      }
  ...
  async function handleFaqAsk() {
    ...
    try {
      ...
      faqAskForm.reset();
      await renderFaqQuestions();
      showNotification('Вопрос отправлен', 'success');
    } finally {
      ...
      if (isSupportChat && !isOwner()) {
        ...
        if (!alreadyAutoAnswered) {
          try {
            ...
            console.error('Auto-reply error:', autoError);
          }
        }
      }
  ...
  async function handleDeleteConversationMessage(messageId) {
    ...
    if (!canDelete) {
      showNotification('Ты не можешь удалить это сообщение', 'warning');
      return;
    }
  ...
  async function handleLogin() {
    ...
    try {
      ...
      if (!state.currentSession) {
        showNotification('Сессия не создалась после входа', 'error');
        return;
      }
  ...
  async function handleAddFolder() {
    ...
    try {
      ...
      if (error) {
        showNotification(error.message || 'Не удалось создать папку', 'error');
        return;
      }
  ...
  async function handleEditFolderCover() {
    ...
    if (!file) {
      showNotification('Выбери новую обложку', 'warning');
      return;
    }
  ...
  async function handleAddPortfolioItem() {
    ...
    try {
      ...
      if (!file) {
        showNotification('Выбери картинку', 'warning');
        return;
      }
      ...
      if (error) {
        showNotification(error.message || 'Не удалось добавить работу', 'error');
        return;
      }
  ...
  async function handleDeleteWork(workId) {
    ...
    if (error) {
      showNotification(error.message || 'Не удалось удалить работу', 'error');
      return;
    }
  ...
  async function handleDeleteReviewReply(replyId) {
    ...
    if (error) {
      showNotification(error.message || 'Не удалось удалить ответ', 'error');
      return;
    }
  
    await renderReviews();
    showNotification('Ответ удалён', 'success');
  }
  ...
  async function startVoiceRecording() {
    if (!navigator.mediaDevices?.getUserMedia) {
      showNotification('Запись голосовых не поддерживается в этом браузере', 'warning');
      return;
    }
    ...
    try {
      ...
      console.error(err);
      showNotification('Не удалось начать запись голосового', 'error');
    }
  }
...
if (updateBioBtn) {
  ...
  (async function init() {
    ...
    style.textContent = `
      ...
      .mkz-notification--success {
        border-left: 4px solid #4caf50;
      }
      
      .mkz-notification--error {
        border-left: 4px solid #f44336;
      }
      
      .mkz-notification--warning {
        border-left: 4px solid #ff9800;
      }
      
      .mkz-notification--info {
...
// ========== ДОБАВЬТЕ ЭТОТ КОД ПЕРЕД ПОСЛЕДНЕЙ СТРОКОЙ ==========

// Функции для описания профиля
...
// Функции для валидации соцсетей
function validateSocialContacts(telegram, vk) {
  ...
  if (hasTelegram) {
    ...
    if (!/^@[a-zA-Z0-9_]{5,32}$/.test(tg)) {
      return { valid: false, message: 'Неверный формат Telegram username. Пример: @username или username (от 5 до 32 символов, только буквы, цифры и _)' };
    }
  }
  ...
  if (hasVk) {
    let vkClean = vk.trim();
    let vkId = vkClean;
    const vkUrlMatch = vkClean.match(/(?:vk\.com\/)([a-zA-Z0-9_.]+)/i);
    if (vkUrlMatch) {
      vkId = vkUrlMatch[1];
    }
    if (vkId.length < 1 || vkId.length > 50) {
      return { valid: false, message: 'Неверный формат ВКонтакте. Укажите ID страницы или username (от 1 до 50 символов)' };
    }
  }
...
// Обновленная функция handleRegister (замените существующую)
// Нужно полностью заменить вашу старую функцию handleRegister на эту
async function handleRegister() {
  if (!registerForm?.reportValidity()) return;
...
// Обновленная функция handleUpdateProfile с сохранением био и приватности
// Замените существующую функцию handleUpdateProfile на эту
async function handleUpdateProfile() {
  if (!state.currentSession) return;
  setButtonState(updateProfileBtn, true, 'Сохранение...', 'Обновить профиль');
  ...
  try {
    ...
    const payload = {
      ...
      email: state.currentSession.user.email || state.currentProfile?.email || '',
      avatar_url: avatarUrl,
      bio: updateBio?.value.trim() || state.currentProfile?.bio || '',
      last_seen_at: new Date().toISOString(),
      show_phone: privacyShowPhone?.checked === true,
      show_telegram: privacyShowTelegram?.checked === true,
      show_last_seen: privacyShowLastSeen?.checked === true,
...
// Обновленная функция renderProfile с описанием
// Замените существующую функцию renderProfile на эту
function renderProfile() {
  const profile = state.currentProfile;
  const name = profile?.username || state.currentSession?.user?.email?.split('@')[0] || 'Гость';
  const phone = profile?.phone || (state.currentSession ? 'Телефон не указан' : 'Не авторизован');
...
// Обновленная функция updateAuthUI (убираем текст про админ-панель)
// Замените существующую функцию updateAuthUI на эту
function updateAuthUI() {
  const loggedIn = !!state.currentSession;
  const owner = isOwner();
...
// Добавляем объявление новых переменных в начало файла (в раздел с DOM элементами)
// Добавьте эти строки туда, где объявлены все const переменные
const updateBio = $('#mkzUpdateBio');
const updateBioBtn = $('#mkzUpdateBioBtn');
const vkInput = $('#mkzVkUsername');
const socialError = $('#mkzSocialError');
...
additionalStyles.textContent = `
  ...
  .mkz-profile-description textarea:focus {
    outline: none;
    border-color: #4f46e5;
